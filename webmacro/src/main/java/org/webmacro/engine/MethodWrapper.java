/*
 * MethodWrapper.java Created on May 24, 2002, 12:01 AM
 */
...
public class MethodWrapper
{
                       ...
                       String methodName)
  {
    ...
    if (mList.size() == 0)
      if (methCnt == 0)
        throw new IllegalArgumentException("No such method as " + methodName + " in class "
                                           + _class.getName() + "!");
      else
        throw new IllegalStateException("Cannot invoke non-static method " + methodName
                                        + " without an instance of the class " + _class.getName()
                                        + "!");
    ...
    for (int i = 0; i < _methods.length; i++) {
      ...
      if (IntrospectionUtils.matches(sig, types)) {
        try {
          ...
          if (obj == null && m.getReturnType() == java.lang.Void.TYPE)
            return org.webmacro.engine.VoidMacro.instance;
          else
            return obj;
        } catch (Exception e) {
          String argList = java.util.Arrays.asList(args).toString();
          String errMsg = "Unable to execute " + getDescription() + " on supplied args: " + argList;
