/*
 * @(#)TemplateServlet.java Copyright (c) 1995-2000 Open Doors Software, Inc. All Rights Reserved.
 * This software is the confidential and proprietary information of Open Doors Software Inc.
 * ("Confidential Information"). You shall not disclose such Confidential Information and shall use
 * it only in accordance with the terms of the license agreement you entered into with Open Doors
 * Software. Open Doors Software MAKES NO REPRESENTATIONS OR WARRANTIES ABOUT THE SUITABILITY OF THE
 * SOFTWARE, EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE IMPLIED WARRANTIES OF
 * MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, OR NON-INFRINGEMENT. OPEN DOORS SOFTWARE SHALL
 * NOT BE LIABLE FOR ANY DAMAGES SUFFERED BY LICENSEE AS A RESULT OF USING, MODIFYING OR
...
/**
 * <p>
 * TemplateServlet provides a servlet to evaluate templates directly as long as the template type or
 * types is mapped to this servlet. (Yes, you can map more than one type in a web.xml file to the
 * same servlet).
 * </p>
 * <p>
 * TemplateServlet offers delegation and global processing. You do not need to subclass this
 * servlet. Implement ServletRouter by setting a configuration attribute. Populate the context in a
 * delegate, not a subclass. This servlet provides pre-processing of a global and request-level
 * template. On servlet initialization, a global template is evaluated once. (This can be
 * refreshed). Since the template may just be creating static blocks, these blocks are then
 * automatically in the context under the name you assign for global processing. Therefore, in your
 * template, you could refer to $Global.CreditCardForm with parameters and evaluate that form as a
 * block. See the wiki page BlockLevelExpressionism for more information on this. Similarly, if your
 * configuration of this servlet specifies a per-Request level template, it will be evaluated and
 * the context placed into the working context prior to evaluation of the actual template.
 * </p>
 * <p>
 * These configuration options make it possible to create automatically a global context and a per
 * request context. A global context means that there is no longer any processing associated with
 * the globals: it is evaluated once. A per-request means that you can always assume that a certain
 * template gets evaluated for every request.
 * </p>
 * <p>
 * This Servlet extends the base class of the JSDK definition eliminating any magic in a
 * super-class.
 * </p>
 * 
 * @since 24 Jan 2004
  ...
  extends HttpServlet
{
  ...
  /**
   * Looks for a global template to evaluate (defined in WebMacro.properties). If defined, the
   * global context is made available to every request as a var specified in WebMacro.properties.
   */
  @Override
  ...
  /**
   * Calls locateTemplate() and then getTemplate(), a base method, to find the template and render
   * it.
   */
  @Override
   ...
   * 
   * @param request
   *          The request which has a uri such as /en/welcome.tml
   * @param response
   *          The response object
  ...
  protected String locateTemplate(HttpServletRequest request)
  {
    ...
    log.info("javax.servlet.include.servlet_path: "
   ...
   */
  protected void loadRequestContext(WebContext context,
                                    String template,
   ...
   * Override as required.
   */
  protected void loadDelegationContext(WebContext context,
                                       String template,
                                       HttpServletRequest request,
                                       HttpServletResponse resp)
      throws ServletException
  {
    if (this.servletRouter != null) {
      try {
        ...
        this.log("Unable to process router " + servletRouter.getClass().getName(), e);
        throw new ServletException(e.toString());
      }
    }
  }
   ...
   */
  protected void refreshGlobalContext()
      throws Exception
  {
    if (globalName == null)
      return;
