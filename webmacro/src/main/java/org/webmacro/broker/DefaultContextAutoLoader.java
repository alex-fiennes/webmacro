  ...
  implements ContextAutoLoader
{
  ...
  /**
   * Load the context tools listed in the supplied string. See the ComponentMap class for a
  ...
  /**
   * Attempts to instantiate the tool using two different constructors until one succeeds, in the
   * following order:
   * <ul>
   * <li>new MyTool(String key)</li>
   * <li>new MyTool()</li>
   * </ul>
   * The key is generally the unqualified class name of the tool minus the "Tool" suffix, e.g., "My"
   * in the example above The settings are any configured settings for this tool, i.e, settings
   * prefixed with the tool's key. <br>
   * NOTE: keats - 25 May 2002, no tools are known to use the settings mechanism. We should create
   * an example of this and test it, or abolish this capability! Abolished -- BG
   */
  private void addAutoVariable(String toolName,
                               String className)
  {
    Class<?> c;
    try {
    ...
    for (int i = 0; i < ctors.length; i++) {
      ...
      if (parmTypes.length == 1 && parmTypes[0].equals(String.class)) {
        ...
        try {
          ...
          _log.error("Failed to instantiate tool " + toolName + " of class " + className
                     + " using constructor " + ctor.toString(), e);
        }
      }
    }
    if (instance == null) {
