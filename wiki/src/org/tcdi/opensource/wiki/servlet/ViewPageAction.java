 ...
 *
 *  If no page name can be determined <i>StartPage</i> property of the specified
 *  WikiSystem is used.<p>
 *
 *  By default, pages are viewed using the <i>ViewPageAction.Template</i>
 *  specified by the WikiSystem, however, one can override the template on a
 *  page-by-page basis by specifying properties in the form of:<pre>
 *   <page name>
 *
 *  .Template </pre>
 *
...
public class ViewPageAction implements PageAction {
    ...
    *
    *  If it is determined that the WikiSystem has defined a unique template for
    *  the specified page, that template is used instead of the default <i>
    *  ViewPageAction.Template</i> .
    *
    * @param  wiki  Description of the Parameter
