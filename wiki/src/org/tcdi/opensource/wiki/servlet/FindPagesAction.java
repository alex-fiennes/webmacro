...
/**
 *  Using the WikiPageFinder of the provided WikiSystem, attempts to find pages
 *  that meet the user's search criteria
 *
 * @author     e_ridge
...
public class FindPagesAction implements PageAction {
   ...
   /**
    *  Use the template specified by <i>FindPageAction.Template</i> in the
    *  WikiSystem's properties
    *
    * @param  wiki  Description of the Parameter
   ...
   /**
    *  Use <code>wiki</code>'s WikiPageFinder to find pages that match the query
    *  found in the <i>find</i> request parameter. Stuffs the WebContext with an
    *  array of <code>WikiPageFinder.FindResult</code> objects as $Results
    *
    * @param  wiki                                Description of the Parameter
    * @param  wc                                  Description of the Parameter
    * @param  user                                Description of the Parameter
   ...
   public void perform(WikiSystem wiki, WebContext wc, WikiUser user, WikiPage page) throws PageAction.PageActionException {
      ...
      if (query.trim().length() > 0) {
         ...
         try {
            ...
            throw new PageAction.PageActionException("Unable to find pages with query=" + query + "; " + e);
         }
      }
   }
}
