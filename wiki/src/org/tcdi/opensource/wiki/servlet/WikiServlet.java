...
public class WikiServlet extends HttpServlet {
   ...
   public void doGet(HttpServletRequest req, HttpServletResponse resp)
      ...
      if (uri.equals("")) {
         ...
         if (uri.equals("/")) {
            uri = uri.concat(_wiki.getProperties().getProperty("StartPage"));
         }
      }
      if (_re.match(uri)) {
    ...
    * @return     a WikiUser object from cookie. null if no cookie or if user
    *      not found
    */
   private WikiUser getUser(WebContext wc) {
      try {
         ...
         if (wc.getRequest().getRemoteUser() != null) {
            uid = wc.getRequest().getRemoteUser().toLowerCase();
            int pos = uid.indexOf("\\");
            if (pos > 0) {
         ...
         if (user != null) {
            ...
            if (autoCreate) {
               ...
               attributes.put("firstname", uid);
               attributes.put("lastname", "");
               attributes.put("comment", "Automatically created from single-sign-on username");

               user = _wiki.registerUser(uid, "User " + uid, "", attributes);
               user.setAttribute("LastAccessed", new Date().toString());
               user.setAttribute("IPAddress", wc.getRequest().getRemoteAddr());
               _wiki.updateUser(user);
            }
   ...
   public final Template handle(WebContext wc) throws HandlerException, PageAction.PageActionException {
      ...
      try {
         ...
         templateName = action.getTemplateName(_wiki, wikiPage);
         return getTemplate(templateName);
      } catch (ResourceException re) {
         throw new HandlerException("Could not get template " + templateName, re);
      }
   }
